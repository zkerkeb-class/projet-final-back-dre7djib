...
export class SubscriptionService {
  ...
  async findAll(): Promise<ResponseSubscriptionDto[]> {
    try {
      ...
      const cachedSubscription =
        await this.cacheManager.get<Subscription[]>(cacheKey);
  ...
  async findOne(id: string): Promise<ResponseSubscriptionDto> {
    try {
      ...
      if (!subscription) {
        ...
        throw new BadRequestException(`Subscription not found with id: ${id}`);
      }
  ...
  async refreshCache(): Promise<void> {
    try {
      ...
      const responseSubscriptions = plainToInstance(
      ...
      for (const subscription of subscriptions) {
        const responseSubscription = plainToInstance(
          ResponseSubscriptionDto,
          subscription,
        );
