...
export class TravelService {
  ...
  async findAllByUserId(userId: string): Promise<TravelResponseDto[]> {
    try {
      ...
      const travels = await this.userModel
        .find({ user_id: userId })
        .lean()
        .exec();
  ...
  async findOne(id: string): Promise<TravelResponseDto> {
    try {
      ...
      if (!travel) {
        ...
        throw new BadRequestException(`Travel not found with id: ${id}`);
      }
  ...
  async update(id: string, updateTravelDto: UpdateTravelDto): Promise<Travel> {
    try {
      const updatedTravel = await this.userModel
        .findByIdAndUpdate(id, updateTravelDto, { new: true })
        .exec();
      ...
      await this.cacheManager.del(
        `travel:user:${updatedTravel.get("user_id")}`,
      );
  ...
  async remove(id: string): Promise<void> {
    try {
      ...
      await this.cacheManager.del("travel:all");
      await this.cacheManager.del(
        `travel:user:${deletedTravel.get("user_id")}`,
      );
    } catch (err) {
